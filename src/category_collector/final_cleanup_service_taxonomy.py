...
INPUT_FILE = (
    ...
    / "service-categories"
    / "taxonomy"
    / "service_taxonomy_final_master_resolved.csv"
)
...
FINAL_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_clean.csv"
COVERAGE_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_clean_coverage.csv"
REMAINING_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_residual_clean.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_clean_summary.csv"
...
def canonical_subservice(raw_service: str) -> str:
    ...
    for pattern, canonical in FINAL_CLEANUP_SUBSERVICE_RULES + RESIDUAL_SUBSERVICE_RULES + LAST_LAP_SUBSERVICE_RULES + ADDITIONAL_SUBSERVICE_RULES + SUBSERVICE_RULES:
        if has(pattern, n):
            return canonical
...
def family_for(raw_service: str) -> str | None:
    ...
    for family, pattern in FINAL_CLEANUP_FAMILY_RULES + RESIDUAL_FAMILY_RULES + LAST_LAP_FAMILY_RULES + ADDITIONAL_FAMILY_RULES + FAMILY_RULES:
        if has(pattern, n):
            return family
