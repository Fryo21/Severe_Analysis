...
INPUT_FILE = (
    ...
    / "service-categories"
    / "taxonomy"
    / "service_taxonomy_final_master_clean.csv"
)
...
FINAL_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_final.csv"
COVERAGE_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_final_coverage.csv"
REMAINING_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_residual_final.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "service_taxonomy_final_master_final_summary.csv"
...
# ============================================================
# FINAL CLEANUP PASS
# Targets the remaining residual clusters from the resolved run.
# ============================================================

FINAL_CLEANUP_SUBSERVICE_RULES = [
    # Loft / attic
    (r"\battic door\b", "Attic Door"),
    (r"\battic hatch\b", "Attic Hatch"),
    (r"\battic trap\b", "Attic Hatch"),
    (r"\bloft hatches?\b", "Loft Hatch"),
    (r"\bloft ladder installation\b", "Loft Ladder Installation"),
    (r"\bloft ladder installer\b", "Loft Ladder Installation"),
    (r"\bloft ladder supplier\b", "Loft Ladder Supply"),
    (r"\bloft storage room\b", "Loft Storage Room"),
    (r"\bloft storage\b", "Loft Storage"),
    (r"\blofts? / loft ladders?\b", "Loft & Loft Ladder Services"),

    # Bedrooms / furniture
    (r"\bbed repairs?\b", "Bed Repair"),
    (r"\bbedroom showroom\b", "Bedroom Showroom"),
    (r"\bbedroom supplier / installer\b", "Bedroom Supply & Installation"),
    (r"^bedrooms?$", "General Bedroom Services"),
    (r"\bcoffee table repair\b", "Coffee Table Repair"),
    (r"\blounge repair\b", "Lounge Furniture Repair"),
    (r"\bsofa bed repairs?\b", "Sofa Bed Repair"),
    (r"\bupholstery repair\b", "Upholstery Repair"),
    (r"^upholstery$", "Upholstery"),
    (r"\bfrench polisher|french polishing\b", "French Polishing"),

    # Beauty / personal care
    (r"\bbody exfoliation\b", "Body Exfoliation"),
    (r"\bjelly pedi\b", "Jelly Pedicure"),
    (r"\blived in blonde\b", "Lived-In Blonde Colouring"),
    (r"^make up$", "Make-Up Services"),
    (r"\bstripey nails\b", "Nail Art"),
    (r"\bspa\b", "Spa Services"),

    # AV / telecom / cabling
    (r"^cabling$", "Cabling Services"),
    (r"\bfreesat\b", "Freesat Installation"),
    (r"\bfreeview\b", "Freeview Installation"),
    (r"\bhead unit installation\b", "Head Unit Installation"),
    (r"\bvolp telephony|voip telephony\b", "VoIP Telephony"),

    # Events / entertainment
    (r"\bbelly dancer\b", "Belly Dance Entertainment"),
    (r"^dancer$", "Dance Entertainment"),
    (r"\bevent staff\b", "Event Staffing"),
    (r"\bevents setting\b", "Event Setup"),
    (r"\bflower wall\b", "Flower Wall Hire"),
    (r"\bmarquee hire\b", "Marquee Hire"),
    (r"\bparty helpers?\b", "Party Assistance"),
    (r"^staffing$", "Staffing Services"),
    (r"\bmusic services?\b", "Music Services"),

    # Doors / access / exterior
    (r"\bcarports? installed\b", "Carport Installation"),
    (r"\bclothesline installation\b", "Clothesline Installation"),
    (r"\bdisability access installation\b", "Accessibility Installation"),
    (r"\bdisabled bathrooms? / showers?\b", "Accessible Bathroom & Shower"),
    (r"\bdormer installation\b", "Dormer Installation"),
    (r"\bflyscreen installation\b", "Flyscreen Installation"),
    (r"\bgarage doors?\b", "Garage Door Services"),
    (r"\bindustrial doors?\b", "Industrial Door Services"),
    (r"\bindustrial garage doors?\b", "Industrial Garage Door Services"),
    (r"\binsect screens?\b", "Insect Screen Installation"),
    (r"\bletterbox replacement\b", "Letterbox Replacement"),
    (r"\bmade to measure doors?\b", "Made-to-Measure Doors"),
    (r"\bperspex / protective screens?\b", "Protective Screen Installation"),
    (r"\bporch installation\b", "Porch Installation"),
    (r"\bporches? repair\b", "Porch Repair"),
    (r"\broller shutters?\b", "Roller Shutters"),
    (r"\bsail shades?\b", "Sail Shades"),
    (r"\bwooden shutters?\b", "Wooden Shutters"),

    # Roofing / chimney
    (r"\bmaster thatcher\b", "Thatching"),
    (r"\bchimneys? repair\b", "Chimney Repair"),

    # Damp / restoration / treatment
    (r"\bcondensation control\b", "Condensation Control"),
    (r"\bfire damage\b", "Fire Damage Restoration"),
    (r"\bflood damage\b", "Flood Damage Restoration"),
    (r"\bodour control\b", "Odour Control"),
    (r"\btimber preservation.*woodworm.*rot\b", "Timber Preservation"),
    (r"\btimber resin repairs?\b", "Timber Resin Repair"),
    (r"\btimber treatment\b", "Timber Treatment"),
    (r"\bwall crack\b", "Wall Crack Repair"),
    (r"\bwoodworm treatment\b", "Woodworm Treatment"),

    # Kitchen / appliances
    (r"\bcooker hood installation.*repair\b", "Cooker Hood Installation & Repair"),
    (r"\bcooktop installation\b", "Cooktop Installation"),
    (r"\bdryer installation\b", "Dryer Installation"),
    (r"\bfreezer installation\b", "Freezer Installation"),
    (r"\bhob installation\b", "Hob Installation"),
    (r"\bimmersion heater installation\b", "Immersion Heater Installation"),
    (r"\bimmersion heater repair\b", "Immersion Heater Repair"),
    (r"\brefrigeration services?\b", "Refrigeration Services"),

    # Driveways / ground / exterior works
    (r"\bdriveways? / patios? / paths?\b", "Driveways, Patios & Paths"),
    (r"\bdrop kerbs?\b", "Dropped Kerb Installation"),
    (r"\bgravel / shingle driveways?\b", "Gravel / Shingle Driveways"),
    (r"^patios?$", "Patio Services"),
    (r"\bslab laying\b", "Slab Laying"),
    (r"\broad markings?\b", "Road Marking"),
    (r"\blevelling\b", "Ground Levelling"),

    # Metal / fabrication / gates
    (r"\bbalustrades?|balustrading\b", "Balustrades"),
    (r"\bdecorative ironmongery.*metalwork\b", "Decorative Metalwork"),
    (r"\blaser cutting stainless steel\b", "Stainless Steel Laser Cutting"),
    (r"\bmetal gates?\b", "Metal Gates"),
    (r"\bmetal works?|metalworks\b", "Metalwork"),
    (r"\brailings?\b", "Railings"),
    (r"\bsteel bending services?\b", "Steel Bending"),
    (r"\bsteel fabricators?\b", "Steel Fabrication"),
    (r"\bsteel work design\b", "Steelwork Design"),
    (r"\bwooden / metal gates?\b", "Wooden & Metal Gates"),
    (r"\bwooden gates?\b", "Wooden Gates"),

    # Waste / clearance / cleaning
    (r"\bchristmas tree disposal\b", "Christmas Tree Disposal"),
    (r"\bdata shredding\b", "Data Shredding"),
    (r"\bmopping\b", "Mopping"),
    (r"\bpallet collection.*disposal\b", "Pallet Collection & Disposal"),
    (r"\brepeat wheelie bin clean\b", "Wheelie Bin Cleaning"),
    (r"\bskip bag hire\b", "Skip Bag Hire"),
    (r"\bsoft washing\b", "Soft Washing"),
    (r"\bvacuumming|vacuuming\b", "Vacuuming"),
    (r"\bwheelie bin clean\b", "Wheelie Bin Cleaning"),
    (r"\bjet / power washing\b", "Jet / Power Washing"),

    # Transport / moving
    (r"\bcommercial deliveries?\b", "Commercial Delivery"),
    (r"\bheavy lifting\b", "Heavy Lifting"),
    (r"\bman and a van|man with a van|man/woman with a van\b", "Man & Van"),
    (r"\bmoving to storage\b", "Move to Storage"),
    (r"\bpackaging service\b", "Packaging Service"),
    (r"\btransportation of heavy vehicles?\b", "Heavy Vehicle Transport"),
    (r"\btransportation of light vehicles?\b", "Light Vehicle Transport"),

    # Vehicles - branded service variants
    (r"\b(fiat|ford|honda|hyundai|jaguar|jeep|kia|land rover|lexus|mazda|mini|nissan|peugeot|porsche|renault|suzuki|toyota|vauxhall|volkswagen|volvo) service\b", "Vehicle Servicing"),
    (r"^garage services?$", "Garage Services"),
    (r"^mot$", "MOT Testing"),
    (r"\bmotor home servicing.*leisure accessories\b", "Motorhome Servicing"),
    (r"^motor homes?$", "Motorhome Services"),
    (r"\bvan conversions?\b", "Van Conversion"),
    (r"\btyres?\b", "Tyre Services"),

    # Digital / business
    (r"\bdevelopment and software\b", "Software Development"),
    (r"\bfile conversion help\b", "File Conversion"),
    (r"\bgoogle analytics\b", "Google Analytics"),
    (r"\bhardware consultants? / technicians?\b", "Hardware Support"),
    (r"\bleaftlet distribution\b", "Leaflet Distribution"),
    (r"\bepos\b", "EPOS Services"),
    (r"\benergy tariff broker\b", "Energy Tariff Brokerage"),
    (r"\binsurance claim specialist\b", "Insurance Claim Assistance"),
    (r"\bproduct testing\b", "Product Testing"),
    (r"^production$", "Production Services"),
    (r"\bqueueing service\b", "Queueing Service"),
    (r"\bebay selling assistance\b", "eBay Selling Assistance"),
    (r"\berrand runners?\b", "Errand Running"),

    # Finance / professional
    (r"\bfinancial advisers?\b", "Financial Advice"),
    (r"\bsurveyors?\b", "Surveying"),
    (r"\btechnical writer\b", "Technical Writing"),

    # Education
    (r"\blanguage classes?\b", "Language Classes"),
    (r"\bdriving instructors?\b", "Driving Lessons"),

    # Health
    (r"^dental$", "Dental Services"),
    (r"\bdentists?\b", "Dentistry"),
    (r"\bdoctors? \(medical practitioners?\)\b", "Medical Practitioner"),
    (r"\bprivate doctors?\b", "Private Doctor"),
    (r"\bpharmacies?\b", "Pharmacy Services"),

    # Property / design / interiors
    (r"^design$", "Design Services"),
    (r"\bhome bar designers?\b", "Home Bar Design"),
    (r"\bhome offices?\b", "Home Office Design"),
    (r"\boffice interior designers?\b", "Office Interior Design"),
    (r"\borangeries?\b", "Orangery Services"),
    (r"\bplanning / design\b", "Planning & Design"),
    (r"\bspace planners?\b", "Space Planning"),
    (r"\bheritage property specialist\b", "Heritage Property Services"),

    # Heating / renewables / fuel
    (r"\bheating & cooling\b", "Heating & Cooling"),
    (r"\bhive heating installers?\b", "Hive Heating Installation"),
    (r"\bradiators? suppliers?\b", "Radiator Supply"),
    (r"\bsmart thermostats?\b", "Smart Thermostat Services"),
    (r"\btado installers?\b", "Tado Installation"),
    (r"\bthermodynamic panels?\b", "Thermodynamic Panels"),
    (r"\bwind turbines?\b", "Wind Turbine Services"),
    (r"\bwind turbines? - feed\b", "Wind Turbine Feed-In Services"),
    (r"\bsolid fuel\b", "Solid Fuel Services"),
    (r"\bsmokeless fuel\b", "Smokeless Fuel Services"),
    (r"\bstored gas\b", "Stored Gas Services"),
    (r"\bstored oil\b", "Stored Oil Services"),

    # Garden / outdoor
    (r"\bjapanese knotweed\b", "Japanese Knotweed Control"),
    (r"\blandscapers?\b", "Landscaping"),
    (r"\bpergolas? installed\b", "Pergola Installation"),
    (r"\bpolytunnels?\b", "Polytunnel Services"),
    (r"\bsheds?\b", "Shed Services"),
    (r"\bsleepers?\b", "Garden Sleepers"),
    (r"\btrellis\b", "Trellis Installation"),
    (r"\btree houses?\b", "Tree House Services"),
    (r"\bturf laying\b", "Turf Laying"),
    (r"\bturf repair\b", "Turf Repair"),
    (r"\bturfing\b", "Turfing"),
    (r"\bwater features?\b", "Water Feature Services"),
    (r"\bwater gardens?\b", "Water Garden Services"),
    (r"\bwhipper snippering\b", "Grass Trimming"),

    # Fire / safety
    (r"\blegionella assessment\b", "Legionella Assessment"),
    (r"\blegionella control\b", "Legionella Control"),
    (r"\blegionella testing\b", "Legionella Testing"),
    (r"\bsprinkler system installation\b", "Sprinkler System Installation"),
    (r"^sprinkler system$", "Sprinkler System Services"),

    # Misc genuine services
    (r"\bcrafts\b", "Craft Services"),
    (r"\bdrawing\b", "Drawing Services"),
    (r"\bgift wrapping\b", "Gift Wrapping"),
    (r"\bmodels?\b", "Model Services"),
    (r"\bmemorial gravestones? supplied\b", "Memorial Gravestone Supply"),
    (r"\btraditional craftspersons?\b", "Traditional Craft Services"),
]


FINAL_CLEANUP_FAMILY_RULES = [
    ("Loft, Attic & Storage Services", r"\b(attic|loft hatch|loft ladder|loft storage|lofts? / loft ladders?)\b"),
    ("Furniture, Carpentry & Woodwork", r"\b(bed repair|coffee table|lounge repair|sofa bed|upholstery|french polish)\b"),
    ("Beauty & Personal Care", r"\b(body exfoliation|jelly pedi|make up|stripey nails|lived in blonde|spa)\b"),
    ("TV, Aerial & Audio Visual Services", r"\b(freesat|freeview|head unit installation)\b"),
    ("Telecoms & Telephone Services", r"\b(cabling|volp telephony|voip telephony)\b"),
    ("Events & Wedding Services", r"\b(belly dancer|^dancer$|event staff|events setting|flower wall|marquee hire|party helpers?|music services?)\b"),
    ("Doors, Windows & Glazing", r"\b(flyscreen|insect screen|industrial doors?|garage doors?|letterbox|made to measure doors?|protective screens?|roller shutters?|wooden shutters?)\b"),
    ("Accessibility & Mobility Services", r"\b(disability access|disabled bathrooms?|wheelchairs?)\b"),
    ("Building & Construction", r"\b(dormer|porch|orangeries?|extensions? / conversions?)\b"),
    ("Roofing Services", r"\b(thatcher|thatching|chimneys? repair)\b"),
    ("Damp, Waterproofing & Restoration", r"\b(condensation|fire damage|flood damage|odour control|timber preservation|timber resin|timber treatment|woodworm|wall crack)\b"),
    ("Appliance & Equipment Services", r"\b(cooker hood|cooktop|dryer installation|freezer installation|hob installation|refrigeration)\b"),
    ("Heating, Boiler & Gas Services", r"\b(immersion heater|heating and cooling|hive heating|radiator supplier|smart thermostat|tado|stored gas|stored oil|solid fuel|smokeless fuel)\b"),
    ("Driveways, Paving & Surfacing", r"\b(driveways? / patios? / paths?|drop kerbs?|gravel / shingle|^patios?$|slab laying|road markings?)\b"),
    ("Metalwork & Fabrication", r"\b(balustrad|decorative ironmongery|laser cutting|metal gates?|metal works?|metalworks|railings?|steel bending|steel fabricat|steel work design)\b"),
    ("Fencing, Gates & Outdoor Structures", r"\b(wooden / metal gates?|wooden gates?|carports?|sail shades?)\b"),
    ("Waste, Clearance & Recycling", r"\b(christmas tree disposal|data shredding|pallet collection|skip bag hire)\b"),
    ("Cleaning Services", r"\b(mopping|wheelie bin clean|soft washing|vacuuming|jet / power washing)\b"),
    ("Removals, Delivery & Courier Services", r"\b(commercial deliveries?|heavy lifting|man and a van|man with a van|man/woman with a van|moving to storage|packaging service)\b"),
    ("Transport & Transfer Services", r"\b(transportation of heavy vehicles?|transportation of light vehicles?)\b"),
    ("Vehicle Services", r"\b((fiat|ford|honda|hyundai|jaguar|jeep|kia|land rover|lexus|mazda|mini|nissan|peugeot|porsche|renault|suzuki|toyota|vauxhall|volkswagen|volvo) service|garage services?|^mot$|motor home|van conversions?|tyres?)\b"),
    ("Software & Development Services", r"\b(development and software)\b"),
    ("IT Support & Device Services", r"\b(file conversion help|hardware consultants?|epos)\b"),
    ("Web & Digital Services", r"\b(google analytics)\b"),
    ("Marketing & Creative Services", r"\b(leaflet distribution|product testing|production)\b"),
    ("Business & Administrative Services", r"\b(insurance claim|ebay selling|errand runners?|queueing service)\b"),
    ("Accounting & Finance Services", r"\b(financial advisers?)\b"),
    ("Architecture, Surveying & Planning", r"\b(surveyors?|planning / design)\b"),
    ("Writing, Translation & Language Services", r"\b(technical writer)\b"),
    ("Tutoring & Lessons", r"\b(language classes?|driving instructors?)\b"),
    ("Health, Therapy & Wellness", r"\b(dental|dentists?|medical practitioners?|private doctors?|pharmacies?)\b"),
    ("Interior Design & Fit-Out", r"\b(home bar designers?|home offices?|office interior designers?|space planners?)\b"),
    ("Property, Landlord & Estate Services", r"\b(heritage property specialist)\b"),
    ("Renewable Energy & Insulation", r"\b(thermodynamic panels?|wind turbines?)\b"),
    ("Gardening & Landscaping", r"\b(japanese knotweed|landscapers?|pergolas?|polytunnels?|sheds?|sleepers?|trellis|tree houses?|turf|water features?|water gardens?|whipper snippering)\b"),
    ("Fire & Safety Services", r"\b(legionella|sprinkler system)\b"),
    ("Printing, Signage & Art Services", r"\b(crafts|drawing|traditional craftsperson|memorial gravestone)\b"),
    ("Home Improvement & General Maintenance", r"\b(disassembly|bike hanging|refinishing|silicone sealing|staining)\b"),
]


FINAL_CLEANUP_NOISE_PATTERNS = [
    r"^(down|how to get started|member t and c'?s apply|united kingdom)$",
    r"^(miscellaneous services|specialist tradesmen|specialist tradesperson|specialist tradespersons|tradesman|trade supplier)$",
    r"^(charity shops|newsagents|supermarkets)$",
    r"^(exeter)$",
]
...
# ============================================================
# FINAL ALIAS PASS
# Small direct-alias layer for obvious residual variants only.
# No new broad taxonomy design here.
# ============================================================

FINAL_ALIAS_SUBSERVICE_RULES = [
    (r"^aluminium gutters?$", "Aluminium Guttering"),
    (r"^aquariums?$", "Aquarium Services"),
    (r"^architraves?$", "Architraves"),
    (r"^artexing$", "Artexing"),
    (r"^awnings? / canopies?$", "Awnings & Canopies"),
    (r"^basement conversions?$", "Basement Conversion"),
    (r"^bathrooms?$", "General Bathroom Services"),
    (r"^bespoke kitchens?$", "Bespoke Kitchens"),
    (r"^cambelts?$", "Cambelt Replacement"),
    (r"^childminders?$", "Childminding"),
    (r"^dry stone walling$", "Dry Stone Walling"),
    (r"^gas engineers?$", "Gas Engineering"),
    (r"^gas fitters?$", "Gas Fitting"),
    (r"^groundworks?$", "Groundworks"),
    (r"^handypersons?$", "General Handyman"),
    (r"^loft conversions?$", "Loft Conversion"),
    (r"^locksmiths?$", "Locksmith Services"),
    (r"^personal trainers?$", "Personal Training"),
    (r"^plasterers?$", "Plastering"),
    (r"^solicitors?$", "Solicitor Services"),
    (r"^telephone engineers?$", "Telephone Engineering"),
    (r"^wet rooms?$", "Wet Room Services"),
    (r"^worktops?$", "General Worktop Services"),
    (r"^writing$", "General Writing Services"),
    (r"^driveways?$", "Driveway Services"),
    (r"^patios?$", "Patio Services"),
    (r"^fencing$", "General Fencing"),
    (r"^roofers?$", "General Roofing"),
    (r"^roofing$", "General Roofing"),
    (r"^plumbers?$", "General Plumbing"),
    (r"^electricians?$", "General Electrical Services"),
    (r"^builders?$", "General Building"),
    (r"^decorators?$", "General Decorating"),
    (r"^painters?$", "General Painting"),
    (r"^gardeners?$", "General Gardening"),
    (r"^landscapers?$", "Landscaping"),
    (r"^cleaners?$", "General Cleaning"),
    (r"^removals?$", "General Removals"),
    (r"^couriers?$", "Courier Services"),
    (r"^tilers?$", "General Tiling"),
    (r"^carpenters?$", "Carpentry"),
    (r"^joiners?$", "Joinery"),
    (r"^surveyors?$", "Surveying"),
    (r"^architects?$", "Architectural Services"),
    (r"^photographers?$", "Photography"),
    (r"^videographers?$", "Videography"),
    (r"^bookkeepers?$", "Bookkeeping"),
    (r"^accountants?$", "Accounting"),
    (r"^translators?$", "Translation"),
    (r"^copywriters?$", "Copywriting"),
    (r"^proofreaders?$", "Proofreading"),
    (r"^web designers?$", "Web Design"),
    (r"^web developers?$", "Web Development"),
    (r"^software developers?$", "Software Development"),
    (r"^it support$", "General IT Support"),
    (r"^computer repairs?$", "Computer Repair"),
    (r"^laptop repairs?$", "Laptop Repair"),
    (r"^tv repairs?$", "TV Repair"),
    (r"^tv installation$", "TV Installation"),
    (r"^aerial installers?$", "Aerial Installation"),
    (r"^barbers?$", "General Barbering"),
    (r"^hairdressers?$", "Hairdressing"),
    (r"^beauticians?$", "Beautician Services"),
    (r"^dog walkers?$", "Dog Walking"),
    (r"^pet sitters?$", "Pet Sitting"),
    (r"^nannies?$", "Nanny Services"),
    (r"^babysitters?$", "Babysitting"),
    (r"^driving instructors?$", "Driving Lessons"),
    (r"^personal training$", "Personal Training"),
    (r"^fitness trainers?$", "Personal Training"),
    (r"^massage therapists?$", "Massage Therapy"),
    (r"^physiotherapists?$", "Physiotherapy"),
    (r"^nutritionists?$", "Nutrition Services"),
]

FINAL_ALIAS_FAMILY_RULES = [
    ("Roofing Services", r"^(aluminium gutters?|roofers?|roofing)$"),
    ("Pet & Animal Services", r"^aquariums?$"),
    ("Furniture, Carpentry & Woodwork", r"^(architraves?|carpenters?|joiners?)$"),
    ("Painting, Decorating & Wall Finishes", r"^(artexing|plasterers?|decorators?|painters?)$"),
    ("Curtains, Awnings & Window Dressings", r"^awnings? / canopies?$"),
    ("Building & Construction", r"^(basement conversions?|builders?|loft conversions?)$"),
    ("Bathroom Services", r"^(bathrooms?|wet rooms?)$"),
    ("Kitchen Services", r"^(bespoke kitchens?|worktops?)$"),
    ("Vehicle Services", r"^cambelts?$"),
    ("Childcare & Care Services", r"^(childminders?|nannies?|babysitters?)$"),
    ("Stone & Masonry Services", r"^dry stone walling$"),
    ("Heating, Boiler & Gas Services", r"^(gas engineers?|gas fitters?)$"),
    ("Groundworks, Excavation & Plant Hire", r"^groundworks?$"),
    ("Handyman, Assembly & Mounting", r"^handypersons?$"),
    ("Locksmith, Keys & Safes", r"^locksmiths?$"),
    ("Fitness & Sports Coaching", r"^(personal trainers?|personal training|fitness trainers?)$"),
    ("Legal Services", r"^solicitors?$"),
    ("Telecoms & Telephone Services", r"^telephone engineers?$"),
    ("Writing, Translation & Language Services", r"^(writing|translators?|copywriters?|proofreaders?)$"),
    ("Driveways, Paving & Surfacing", r"^(driveways?|patios?)$"),
    ("Fencing, Gates & Outdoor Structures", r"^fencing$"),
    ("Plumbing & Drainage Services", r"^plumbers?$"),
    ("Electrical Services", r"^electricians?$"),
    ("Gardening & Landscaping", r"^(gardeners?|landscapers?)$"),
    ("Cleaning Services", r"^cleaners?$"),
    ("Removals, Delivery & Courier Services", r"^(removals?|couriers?)$"),
    ("Flooring & Tiling Services", r"^tilers?$"),
    ("Architecture, Surveying & Planning", r"^(surveyors?|architects?)$"),
    ("Photography & Video Services", r"^(photographers?|videographers?)$"),
    ("Accounting & Finance Services", r"^(bookkeepers?|accountants?)$"),
    ("Web & Digital Services", r"^(web designers?|web developers?)$"),
    ("Software & Development Services", r"^software developers?$"),
    ("IT Support & Device Services", r"^(it support|computer repairs?|laptop repairs?)$"),
    ("TV, Aerial & Audio Visual Services", r"^(tv repairs?|tv installation|aerial installers?)$"),
    ("Hair & Barber Services", r"^(barbers?|hairdressers?)$"),
    ("Beauty & Personal Care", r"^beauticians?$"),
    ("Pet & Animal Services", r"^(dog walkers?|pet sitters?)$"),
    ("Tutoring & Lessons", r"^driving instructors?$"),
    ("Health, Therapy & Wellness", r"^(massage therapists?|physiotherapists?|nutritionists?)$"),
]
...
def canonical_subservice(raw_service: str) -> str:
    ...
    for pattern, canonical in FINAL_ALIAS_SUBSERVICE_RULES + FINAL_CLEANUP_SUBSERVICE_RULES + RESIDUAL_SUBSERVICE_RULES + LAST_LAP_SUBSERVICE_RULES + ADDITIONAL_SUBSERVICE_RULES + SUBSERVICE_RULES:
        if has(pattern, n):
            return canonical
...
def family_for(raw_service: str) -> str | None:
    ...
    for family, pattern in FINAL_ALIAS_FAMILY_RULES + FINAL_CLEANUP_FAMILY_RULES + RESIDUAL_FAMILY_RULES + LAST_LAP_FAMILY_RULES + ADDITIONAL_FAMILY_RULES + FAMILY_RULES:
        if has(pattern, n):
            return family
...
def is_noise(raw_service: str) -> bool:
    ...
    return any(has(pattern, n) for pattern in NOISE_PATTERNS + FINAL_CLEANUP_NOISE_PATTERNS)
